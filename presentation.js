...
app.post("/login", async (req, res) => {
    ...
    let emptyFields = await business.checkEmptyFieldsLogin(usernameInput, passwordInput)
    if (emptyFields) {
        ...
        let user = await business.checkUserCredentials(usernameInput, passwordInput)
        if (user){
            return res.render("landing_page", {layout:undefined})
        }
        else{
            return res.render("login", { 
                layout: undefined, 
...
app.post("/register", async (req, res) => {
    ...
    let emptyFields = await business.checkEmptyFieldsRegister(firstnameInput, lastnameInput, emailInput, usernameInput, passwordInput, repeatPasswordInput)
    ...
    if (passwordComplexity.test(passwordInput)&&firstnameContainsLetters&&lastnameContainsLetters&&!emptyFields&&passwordsMatch) {
        let userDetails = {
        ...
        let user = await business.sendUserDetails(userDetails)
